...
Inbox badge is live — unread agent decisions plus anything overdue or due today.
...
subject or topic, add an entry here — nothing else needs touching.
...
(`generate_story_question`, its own narrator persona, unchanged) and an enemy
portrait per level — everywhere else, the AI's lead-in is a plain "how to
approach it" pointer, not a story:

<p align="center"><img src="docs/screenshots/story-challenge.png" width="620" alt="A Story Mode level — narrative flavour and a portrait, kept as its own thing"></p>
...
A single Vercel **Node** function (`maxDuration: 60`) — the only place the
...
sync if you change a prompt; the deployed app only calls `/api/ai`.
...
All tunables live in [src/study/config.ts](src/study/config.ts) — explicitly
MVP calibration, not permanent.

**Mastery** ([mastery.ts](src/study/mastery.ts)) — a deterministic rolling
...
**Plan confidence** ([confidence.ts](src/study/confidence.ts)) — the number the
...
**Strategy ladder** ([strategy.ts](src/study/strategy.ts)) — one rung per tick
only (`NORMAL ↔ STRUGGLING ↔ PERSISTENT`):
...
jump, or name a topic outside the mission, and the **whole** decision is
rejected and logged — never a partial apply.
...
**Idempotency**: `agent_events` has `UNIQUE(mission_id, trigger_id)` — the same
session completion, the same day's tick, or a double click each apply *at most
once*. **Failure safety**: every failure mode (Strands unreachable, bad
decision JSON, validator rejection, a Supabase write failing mid-apply) leaves
the plan unchanged and logs why — never a partial apply, never a false success.
...
| [0005_onboarding.sql](supabase/migrations/0005_onboarding.sql) | — | `profiles.onboarded_at`, for the first-run wizard. |
| [0006_routine_cadence.sql](supabase/migrations/0006_routine_cadence.sql) | — | Widens `routines.cadence` to `daily`\|`weekly`\|`biweekly`\|`monthly`. |
| [0007_calendar_time.sql](supabase/migrations/0007_calendar_time.sql) | — | Adds `calendar_events.event_time` / `routines.time_of_day` (`"HH:MM"` text, regex-checked) — a date alone was never enough. |
...
- **[Inbox](src/screens/Inbox.tsx)** (`/inbox`) — a digest line (the latest
  `DAILY` run), **Needs you** (unread `notifications` — the agent couldn't
  decide alone), and **On your plate**
  ...
  item; each active routine gets one static line instead (e.g. *"Every Tuesday
  at 6:00 PM"*).
...
| [`/api/agent/decide.py`](api/agent/decide.py) | Bedrock (Claude) via Strands | Proposes plan changes as one closed-contract JSON. | **No** — read-only tools, gate does the writing. |
...
**Auth** — email magic-link ([src/auth/](src/auth/)). No env vars → **local
...
supabase/migrations/   0001 … 0007  (see Data model above)
...
**90 tests**, all deterministic logic — nothing that needs a model or a DB:
...
<p align="center"><img src="docs/screenshots/settings.png" width="620" alt="Settings — Arcade skin, colour themes, Story Mode portraits toggle"></p>
